		...
		if ($_SERVER['REQUEST_METHOD']=='POST') {
            ...
            if ($_SERVER['REQUEST_METHOD']== 'POST') {
                if (isset($_POST['submit'])) {
					...
					$acc_type = mysqli_real_escape_string($conn,strip_tags($_POST['acc_type']));
                    $acc_type2 = mysqli_real_escape_string($conn,strip_tags($_POST['acc_type2']));
                    $branch = mysqli_real_escape_string($conn,strip_tags($_POST['branch']));
                    $acc_no = mysqli_real_escape_string($conn,strip_tags($_POST['acc_no']));
                    $cacc_no = mysqli_real_escape_string($conn,strip_tags($_POST['cacc_no']));
                    $check = "SELECT id FROM admin_detail WHERE email ='$email'";
                    ...
                    if ($row > 0) {
                        ...
                        $query = "INSERT INTO `admin_detail` (`fullname`,`email`,`username`,`password`,`acc_type`,`acc_type2`,`branch`,`acc_no`,`cacc_no`)
                         VALUES(?,?,?,?,?,?,?,?,?)";
                         $stmt = $conn->prepare($query);
                         $stmt->bind_param('sssssssii',$fullname,$email,$username,$password,$acc_type,$acc_type2,$branch,$acc_no,$cacc_no);
									...
									<div class="col-md-9 bg-light my-3">
                                        <div class="card shadow">
                                                ...
                                                <form action="" method="POST" class="form-group">
													...
													<input class="form-check-input" type="checkbox" name="acc_type" value="Saving Account" id="flexCheckDefault">
													<label class="form-check-label" for="flexCheckDefault">
														Saving Account
													</label>
													</div>
													<div class="form-check d-inline">
													<input class="form-check-input" type="checkbox" name="acc_type2" value="Current Account" id="flexCheckChecked">
													<label class="form-check-label" for="flexCheckChecked">
													Current Account
													</label>
													</div>
													<label class="d-block mt-4" for="branch">Branch</label>
                                                    <input type="text" required name="branch" class="form-control"><br>
													<label for="acc_num">Saving Account Number</label>
                                                    <input type="number" required name="acc_no" class="form-control"><br>
                                                    <label for="cacc_num">Current Account Number</label>
                                                    <input type="number" required name="cacc_no" class="form-control"><br>
